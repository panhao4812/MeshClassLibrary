...
namespace MeshClassLibrary
{
    ...
    {
        public List<Line> profile(Mesh mesh)
        {
            ...
            for (int k = 0; k < PointList.Count; k++)
            {
                ...
                {
                    if (PointList[i].order == 1)
                    {
                        sign = false;
                        PointList[i].order ++;
                        if (PointList[i].refpoints.Count == 4)
                        {
                            if (PointList[i].refpoints[0].order > 0) { PointList[i].refpoints[2].order++; }
                            if (PointList[i].refpoints[1].order > 0) { PointList[i].refpoints[3].order++; }
                            if (PointList[i].refpoints[2].order > 0) { PointList[i].refpoints[0].order++; }
                            if (PointList[i].refpoints[3].order > 0) { PointList[i].refpoints[1].order++; }
                        }
                        else
                        {                       
